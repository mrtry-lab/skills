...
metadata:
    github-path: skills/agile-refine-implementation-plan
    github-ref: refs/heads/main
    github-repo: https://github.com/mrtry-lab/skills
    github-tree-sha: ee9b7bbaf488050956a76321f4ef1cf36ef998e3
...
**Status 遷移**: Implementation Plan Issue は Step 14 で起票するため、本ステップ時点では Issue 自体がまだ存在しない (Step 14 で `In Code Review` 起票)。
...
## Step 14: Implementation Plan Issue 起票
...
更新コマンド: `bash ~/.claude/skills/agile-update-skills/scripts/update-issue-status.sh <story-issue-number> "In Coding Progress"` を実行。失敗時はリトライ 1 回、それでも失敗すれば手動更新を案内して作業を続行 (Status 更新で本作業をブロックしない)。
...
- **Implementation Plan Issue 起票実行** — Step 14 の `/agile-create-issue` 委譲前の最終確認
