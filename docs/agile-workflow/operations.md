...
| Ready | Refinement 完了。**Plan/Task 未作成** | Plan Issue 起票完了、Refinement 待ち | 仕様確定、実装着手可能 |
| In Coding Progress | **Plan or Task が作成済み (実装フェーズ入り)** | Plan 本文を編集中 (Refinement 継続) | CodingAgent が実装中 |
| In Code Review | (使わない) | Plan レビュー中 (Dev + PdO + QA) | PR レビュー中 |
| Done | 受入確認完了、全 Sub-issue Done | Plan 承認完了 | PR マージ完了 |

**Plan Issue は Task と同じ 4 Status (`Ready` / `In Coding Progress` / `In Code Review` / `Done`) を使う**。これにより Sprint ビューに Plan も Story 配下で Task と並んで表示され、レビュー進捗を統一的に追える。Plan は PR を出さないが、`In Code Review` は「Plan ドキュメントのレビュー (Dev + PdO + QA)」を意味する。

`agile-refine-implementation-plan` Step 14 は **Refinement 完了済みの内容で Plan Issue を起票** するフローなので、起票時の Status は `In Code Review` (レビュー待ち) になる。再 Refinement が必要なら `In Coding Progress` に戻して編集 → `In Code Review` に戻す。
...
| Plan Issue が `In Planning` / `In Plan Refinement` / `In Plan Review` に遷移しそうになる | Plan は Task と同じ 4 Status (Ready / In Coding Progress / In Code Review / Done) を使う。Story 系の Plan-prefix 付き Status は Story 専用 |
