...
運用上、Project のビューは **Backlog / Sprint の 2 つだけ** にする。Project 作成時にデフォルトで作られる `View 1` / `Table` 等は削除し、以下 2 つだけが残る状態を維持する:

- **Backlog** — Group by: **Parent issue (Epic)** / Filter: `is:open status:"In Planning","In Plan Refinement","In Plan Review","Ready","In Coding Progress","Done" type:"Epic","Story"`
  Open な Epic 配下の Story を Epic 別に俯瞰するビュー。**Epic と Story だけを表示** することで「どの大きな塊にどの Story があるか」が一覧できる。Implementation Plan / Task は Sprint で追う。Epic が Done になると Sub-issue all closed → Parent auto-close で連鎖 close され、`is:open` フィルタで Backlog から自動的に外れる。

- **Sprint** — Group by: **Parent issue (Story)** / Filter: `status:"Ready","In Coding Progress","In Code Review","Done" type:"Story","Implementation Plan","Task"`
  実装フェーズに入った Story 配下の Implementation Plan / Task を Story 別に追うビュー。**Story / Implementation Plan / Task だけを表示** することで、Epic レベルのチャンクを除外して「いま手を動かしている塊」に集中できる。

両ビューに `Ready` / `In Coding Progress` の Status と Story Issue Type が重複表示されるが、視点が違うため OK (Backlog では Epic 配下の俯瞰、Sprint では Story 配下の実装追跡)。
...
| Story が `Ready` のまま Task 起票が進んでいる | Implementation Plan 起票 / 最初の Task 起票時の自動遷移が失敗している。`agile-refine-implementation-plan` Step 14 後 または `agile-decompose-task-from-implementation-plan` Step 5 後の Status 更新コマンドが正常終了したか確認、必要なら手動で In Coding Progress に更新 |
