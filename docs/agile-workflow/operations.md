...
- **Sprint** — Layout: **Board** / Group by: **Parent issue (Story)** / Filter: `status:"Ready","In Coding Progress","In Code Review","Done" type:"Implementation Plan","Task"`
  実装フェーズに入った Story 配下の Implementation Plan / Task を Story 別に追うビュー。**表示されるのは Implementation Plan と Task だけ**で、Story は Group by の親ヘッダーとして見える。**子 (Plan/Task) を持たない Story はヘッダー自体が生成されず Sprint に出ない** ので、Refinement だけ済んで実装着手前の Story はノイズとして並ばない。

- **Overview** — Layout: **Table** / Group by: **Type** / Filter: `is:open` / Show hierarchy: **On** / 表示フィールド: Title / Type / Status / Sub-issues progress
  全 Issue を Type 別に階層付きで俯瞰するビュー。Backlog / Sprint が「ある視点のスライス」なのに対し、Overview は **プロジェクトの全体像** を見るためのもの。Epic → Story → Plan/Task の階層と各 Issue の完了率 (Sub-issues progress) が一覧できる。
