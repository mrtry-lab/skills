...
Plan は Task と同じ 4 Status を使う。これにより Sprint ビュー (Filter: `Ready` / `In Coding Progress` / `In Code Review` / `Done`) に Plan も表示され、Story 配下で Plan と Task のレビュー進捗を統一的に追える。Plan は PR を出さないが、`In Code Review` は「Plan ドキュメントのレビュー (Dev + PdO + QA)」を意味する。

| Plan Status | 意味 |
|---|---|
| Ready | Plan Issue 起票完了、Refinement 待ち (現状の `agile-refine-implementation-plan` フローでは通常使わない) |
| In Coding Progress | Plan 本文を編集中 (Refinement 継続) |
| In Code Review | Plan レビュー中 (Dev + PdO + QA) |
| Done | Plan 承認完了 |

`agile-refine-implementation-plan` は Step 1-13 で Refinement を進めて Step 14 で **Refinement 完了済み内容を Plan Issue として起票** する流れなので、起票時の Status は `In Code Review` (レビュー待ち) になる。再 Refinement が必要なら `In Coding Progress` に戻して編集 → `In Code Review` に戻す。
...
```
Story #45 (Status: In Plan Refinement → ... → In Coding Progress; Plan 起票で遷移済)
 ├─ Plan #46 (Status: In Code Review → Done)
 │   ↓ Plan Done を受けて起票
 ├─ Task #47 (Ready)
 ├─ Task #48 (Ready)
 └─ Task #49 (Ready)
...
Plan と Task は Sprint ビュー (Filter: Ready / In Coding Progress / In Code Review / Done) で Story 配下に並んで表示される。
