...
シーケンス図がアクター間のフローを網羅するのに対し、Example Mapping は **ビジネスルール（誰がいつ何を許可されるか、どの値が有効か、状態遷移の制約）の網羅** に強い。両者は補完関係で、`agile-refine-story` の Step 5f として組み込まれている。
...
|---|------|---------------------|
| 黄 User Story | 題目 | Story 本体（refine-story Step 1 で読み込み済み） |
...
タイムボックスは `team-context.json` のプリセットに従う（軽量: 25 分 / 標準: 30-60 分 / 集中: 60-90 分）。超過するパターンは決まっていて:

- ルール **5 個超** → Story の責務が広がりすぎ。`/agile-create-stories` に戻って分割
