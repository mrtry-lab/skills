...
- **`agile-decompose-task-from-implementation-plan`**: 分解せず Story を 1 Task として扱い、即 `agile-implement-task` へ
- **`agile-implement-task`**: 通常通り。ただし TDD を妥協しない（hotfix → 安定化後にテストを必ず追加）
