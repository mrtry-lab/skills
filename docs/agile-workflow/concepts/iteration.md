...
## 新規 Project setup での扱い

新規 Project は **公開テンプレート `mrtry-lab/Agile Project Sample` ([https://github.com/orgs/mrtry-lab/projects/3](https://github.com/orgs/mrtry-lab/projects/3))** を `copyProjectV2` mutation で複製するのが最速。Iteration field (duration=180 / iterations[0]) もコピー元から引き継がれるので、setup 後すぐに `iteration:@current` Sprint Filter が機能する。

`/agile-setup-project` の Step 4a (Template 経路) を選べばこの流れが自動。複製後に `assert-iteration-field.sh` で `duration=180` / `iterations >= 1` を検査して exit 0 を確認。

## 既存 Project の migration

`iteration_field` が設定されていない (= 旧 setup) 既存 Project は以下が必要:
...
setup スキルを再実行すれば対話的に進められる。新規 Project を作り直す余裕があれば Template 経路に切り替える方が楽。

## 参考

- `shared/references/github-projects.json.template` — `iteration_field` セクション
- `skills/agile-setup-project/SKILL.md` Step 4a / Step 5 — Template 経路と手動経路の両方
- `skills/agile-setup-project/scripts/copy-from-template.sh` — Template 複製 + 各種 ID 抽出
- `skills/agile-setup-project/scripts/assert-iteration-field.sh` — Iteration field 構成検査
- `skills/agile-setup-project/references/browser-iteration-field.md` — Web UI 操作 fallback (Template 不使用パス)
