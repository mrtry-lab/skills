...
flowchart TB
  ...
  plan_decision{"Plan 必要?"}

  subgraph plan_phase["⑥ Implementation Plan 作成 (Plan 必要パスのみ)"]
    p1["/agile-refine-implementation-plan\nPlan Issue を Story の sub-issue として起票"]
    p2["Status: In Plan Refinement → Done"]
    p1 --> p2
  end

  subgraph taskphase["⑦ タスク分解 — Status: Ready"]
    t1["/agile-implementation-plan-to-task"]
  ...
  plan_decision -- "Plan 必要" --> p1
  plan_decision -- "Plan 不要 (軽量パス)" --> t1
  p2 --> t1
  t2 --> c1
...
`nature:chaotic` の Story は Step 1.5 軽量フローで Refinement → Ready → 直接 `/agile-task-implementation` に直行する（Plan も Task 分解もスキップ。詳細は [concepts/cynefin.md](concepts/cynefin.md)）。

Plan 必要性の判定基準は [concepts/implementation-plan.md](concepts/implementation-plan.md) 参照。
...
- Issue 階層は **Epic → Story → (Implementation Plan + Task) の 3〜4 層**。Plan と Task は Story の直下 sub-issue として並列
...
### 4. `/agile-refine-backlog` — Story リファインメント (PdO/QA 視点)
...
**判定後の分岐**:
- Plan 必要 → `/agile-refine-implementation-plan`
- Plan 不要 (軽量パス) → `/agile-implementation-plan-to-task` (Story 入力モード)
...
Plan の責務・判定基準は [concepts/implementation-plan.md](concepts/implementation-plan.md) 参照。

### 6. `/agile-implementation-plan-to-task` — タスク分解

Implementation Plan から Task Issue を起票。Plan の「Task 分解 (PR 計画)」を踏襲。

**2 つのモード**:
- **Plan ベースモード**: Plan Issue を入力に、Plan の Task 分解を踏襲して Task 起票
- **軽量モード**: Plan 不要の軽量 Story の場合、Story から直接 Task 起票
...
- **フロー**: Task Issue 読み込み + 関連 Plan 確認 → Plan mode で計画 → 計画品質スコアリング → ナビゲーター承認 → TDD 実装 → 検証 → Draft PR
...
定例（軽い同期）
  ...
  ├── 必要なら Story リファインメント (/agile-refine-backlog)
...
`agile-create-backlog` と `agile-refine-backlog` で共通のテンプレートを使い、段階的に TBD を埋めていく。Story は **PdO/QA 視点 (What/Why)** に絞られている。
...
**refine-backlog 段階で埋める項目**: ユーザー体験フロー (概念)、Outcome Done、ビジネスルール、未解決の質問、詳細な受入基準

> エンジニア視点の詳細 (技術詳細シーケンス図、API 仕様、データモデル、画面詳細、ロギング実装、テスト戦略、Task 分解、横断的判断) は **Implementation Plan** に分離されている。Plan が必要な Story はその起票時に `/agile-create-issue` が `templates/implementation-plan.md` を使う。

## Implementation Plan Issue テンプレート

Story の sub-issue として起票される Plan Issue は、エンジニア視点の戦略を集約する:
