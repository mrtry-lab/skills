...
`nature:chaotic` の Story は Step 2 軽量フローで Refinement → Ready → 直接 `/agile-implement-task` に直行する（Implementation Plan も Task 分解もスキップ。詳細は [concepts/cynefin.md](concepts/cynefin.md)）。
...
**10 ステップ**:
1. Story Issue 読み込み
2. chaotic 軽量フロー (該当時のみ)
3. ビジョン整合レビュー (PdO 視点)
4. ユーザー体験フロー (概念レベル)
5. Outcome Done 定義
6. Example Mapping (ビジネスルール)
7. 受入基準生成
8. PdO + QA Three Amigos 並列網羅性検査
9. Implementation Plan 必要性判定
10. 次スキル案内
...
- chaotic → `/agile-implement-task` 直行
...
**16 ステップ**:
1. 親 Story Status の Hard gate
2. 副チェック (Implementation Plan 必要性再判定)
3. Story 読み込み + 既存実装探索
4. 技術詳細シーケンス図
5. API 仕様詳細
6. データモデル
...
11. 横断的判断
12. 意図的に扱わないこと
13. 品質スコアリング (8 点、7 点以上で合格)
14. Three Amigos レビュー (Dev + PdO + QA、Dev がメイン責務)
15. Implementation Plan Issue 起票
16. 次スキル案内
...
### 8. `/agile-sprint-review` — 受け入れ確認

子 Plan/Task が全 Done になった Story (`Awaiting sprint review` Status) を 1 件ずつ AC / Outcome / 関連 PR を walkthrough しながら verify するスキル。

- skill 起動時に `In Coding Progress` の Story を全 scan し、子が全 Done のものを `Awaiting sprint review` に拾い上げる (lazy scan / 取りこぼし回収)
- 各 Story を 1 件ずつ提示し、`AskUserQuestion` で OK / Reject / Skip を判定
- OK → Story Status=Done に進めて子 Plan/Task の iteration を外す (Sprint Board から落とす)
- Reject → Story Status=In Coding Progress に戻し、追加 Task 起票を案内
- Skip → 保留、次回再候補

「Scrum セレモニーとして固定で回す」のではなく、「`Awaiting sprint review` が溜まってきたタイミングで都度回す」運用前提。

### 9. `/agile-setup-project` — プロジェクトセットアップ

`agile-*` スキル群が前提とする GitHub Project (v2) の構成を、対話で 1 回流すだけで揃える。

- Issue Type の登録確認 (Epic / Story / **Implementation Plan** / Task) — `assert-issue-types.sh` で自動検査
- **GitHub Project (v2) の用意 — 3 経路 (Recommended は Template コピー)**:
  - **Template からコピー (Recommended)**: 公開済み Template Project `mrtry-lab/Agile Project Sample` (https://github.com/orgs/mrtry-lab/projects/3) を `copyProjectV2` mutation で 1 リクエスト複製。Status 8 options / Iteration field 180 日 / 3 Views / Workflows が**初期状態で揃った状態**で立ち上がる
  - 新規作成: 空 Project を作って Status / Iteration field / Workflows / Views を手動 setup
  - 既存 Project の取り込み: 既に手動構成した Project を使う
- 引き継ぎ状況の自動検査 (Template 経路) — Status options / Iteration field / Workflows / Views を `assert-*.sh` 4 種類で個別 assert、不足項目だけ手動 fallback に降りる
...
### 10. `/agile-update-skills` — 一括最新化

agile-* スキル群と `docs/agile-workflow/` を一括で最新化する。`agile-setup-project` の Step 9 から委譲呼び出しされる他、定期実行 (月 1 回程度) で agile-* の最新版に追従できる。

- `gh skill install` で 12 スキル (11 + 自己) を上書きインストール
