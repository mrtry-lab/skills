...
`nature:chaotic` の Story は Step 1.5 軽量フローで Refinement → Ready → 直接 `/agile-implement-task` に直行する（Implementation Plan も Task 分解もスキップ。詳細は [concepts/cynefin.md](concepts/cynefin.md)）。
...
agile-* スキル群と `docs/agile-workflow/` を一括で最新化する。`agile-setup-project` の Step 7.5 から委譲呼び出しされる他、定期実行 (月 1 回程度) で agile-* の最新版に追従できる。
