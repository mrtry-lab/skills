...
flowchart TB
  ...
  pre --> it --> proj --> status --> done --> views --> refs --> summary
```
...
# タスク分割単位（4 フィールド）— ヒアリング結果で置換
sed -i '' \
  -e 's|<MONOREPO / MULTI_REPO>|MONOREPO|g' \
  -e 's|<USE_CASE / LAYER / COMPONENT / VERTICAL_SLICE / CUSTOM>|USE_CASE|g' \
  -e 's|<INLINE / SEPARATE_PR / N_A>|SEPARATE_PR|g' \
  -e 's|<例: 1 ユースケース分の BE+FE 統合 PR / BE か FE か Infra のいずれかのレイヤ 1 つ>|1 ユースケース分の BE+FE 統合 PR、DB migration は別 PR|g' \
  .claude/skills/references/team-context.md
```
