...
- プロダクトの方向性自体が未定義（→ `/agile-product-vision`）
- EpicをStoryに分解したい（→ `/agile-create-backlog`）
- 個別Storyの詳細化（→ `/agile-refine-backlog`）
...
- **0→1**: `docs/VISION.md` が存在し、Issue Type `Epic` のIssueがまだない → VISION.mdのLayer 3（What）から機能クラスターを抽出し、Epic候補を提示
...
VISION.mdのLayer 3（What: ユーザーの課題、Not-to-doリスト、成功指標）を読み込み、以下を問いかける:
...
- **ビジョンとの整合**: 「VISION.mdのミッション・Not-to-doリストと整合しているか?」→ Not-to-doの「やらないこと」に該当するなら、それでも進める根拠を確認
...
- 深掘り: 「VISION.mdのペルソナと一致するか?」「全ユーザー共通の課題か、特定セグメントだけか?」
...
- 深掘り: 「その指標はユーザーの行動に紐づいているか?（ページビューではなく、完了率やリテンション率など）」
...
**サブエージェントへの指示**:
```
以下の Epic Issue の Opportunity Canvas 内容と docs/VISION.md を読み込み、検査してください。
...
- 「このEpicをStoryに分解するには `/agile-create-backlog` を使ってください」
...
| 副業チームでEpicが5つ以上になりそう | 「同時に進められるEpicは2-3個が限界。優先順位をつけましょう」と促す |
...
- **NEVER: 副業チームでアクティブなEpicを5つ以上持つな** — 限られた稼働時間が全方位に分散し、どのEpicも完成しない
