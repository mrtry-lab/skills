...
`Epic` / `Story` / `Implementation Plan` / `Task` の 4 つの Issue Type が Organization に登録されている必要がある。**登録自体は GraphQL API / gh CLI とも未対応**で Web UI 操作が必須だが、**登録済みか否かの判定は GraphQL で可能** なので assertion script を使う。

### 1. assertion で現状チェック

```bash
bash <skill-dir>/scripts/assert-issue-types.sh --org <ORG>
```

- **exit 0**: 4 種類すべて登録済み → Step 3 へ
- **exit 1**: 不足あり、stdout に `MISSING_ISSUE_TYPES=Story,Task` のように出る → Step 2-2 で登録案内
- **exit 2**: Org が見つからない → ユーザーに確認

### 2. 未登録分の登録案内 (exit 1 のときのみ)

`MISSING_ISSUE_TYPES` の値をユーザーに伝え、`AskUserQuestion` で「browser で登録するか / 手動でやるか」を聞く。

**browser で操作**: `Read` で `<skill-dir>/references/browser-issue-types.md` を読み込み、不足分だけ登録する。

**手動 fallback**:

> Organization Settings → Planning → Issue types に移動し、不足分 (`<MISSING_ISSUE_TYPES>`) を登録してください。
> URL: `https://github.com/organizations/<ORG>/settings/issue-types`
>
> 色やアイコンの選択肢は自由。

登録後、もう一度 `assert-issue-types.sh` を流して exit 0 を確認してから Step 3 へ。
...
#### 4a-3. 引き継ぎ状況を 4 つの assertion script で検査

各 assert スクリプトを並列に呼び、exit code と stdout で不足を判定:

```bash
# Status options 8 件揃ってるか
bash <skill-dir>/scripts/assert-status-options.sh --owner <DEST_OWNER> --number <NUMBER>
STATUS_OK=$?

# Iteration field (duration=180, current iteration あり)
bash <skill-dir>/scripts/assert-iteration-field.sh --owner <DEST_OWNER> --number <NUMBER>
ITERATION_OK=$?

# Workflows (Item closed ON / Auto-add OFF / Auto-close OFF)
bash <skill-dir>/scripts/assert-workflows.sh --owner <DEST_OWNER> --number <NUMBER>
WORKFLOWS_OK=$?

# Views (Backlog / Sprint / Overview)
bash <skill-dir>/scripts/assert-views.sh --owner <DEST_OWNER> --number <NUMBER>
VIEWS_OK=$?
```

各 script は exit 0 / 1 を返し、不足があるときは stdout に `MISSING_STATUS_OPTIONS=...` / `ITERATION_FIELD_MISSING=true` / `WORKFLOW_AUTO_CLOSE_ENABLED=true` / `MISSING_VIEWS=...` のような env-var を出力する。

判定:

- **全 exit 0** → 引き継ぎ完了。**Step 5 / 6 / 7 をスキップして Step 8 (shared references 生成) に直行**
- **exit 1 が混じる** → 不足項目をユーザーに提示し、対応する Step に降りる:
  - `STATUS_OK != 0` or `ITERATION_OK != 0` → **Step 5** (Status options + Iteration field) のうち不足分を実行
  - `WORKFLOWS_OK != 0` → **Step 6** (Workflows 設定) を実行
  - `VIEWS_OK != 0` → **Step 7** (Views 作成) を実行
...
一覧から対象を選んでもらい、その Number を控える。Step 4a-3 と同じ 4 つの assert スクリプト (`assert-status-options.sh` / `assert-iteration-field.sh` / `assert-workflows.sh` / `assert-views.sh`) を流して不足項目を検出 → 該当 Step (5/6/7) だけ降りる、という流れが楽。
...
各 Issue の検証は **`assert-issue-state.sh`** で一括 assert する (Issue Type / Status / parent / iteration を 1 コマンドで)。失敗時は stdout に `MISMATCH_*` が出るので、ユーザーに提示して原因切り分け。

```bash
SCRIPT_DIR=<skill-dir>/scripts
bash $SCRIPT_DIR/assert-issue-state.sh <num> \
  --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> \
  --type <ExpectedType> --status <ExpectedStatus> \
  [--parent <ParentNum>] [--has-iteration|--no-iteration]
```
...
**検証**:
```bash
bash $SCRIPT_DIR/assert-issue-state.sh <epic-num> --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> --type Epic
```

(Epic は Status を持たない設計なので type だけ check)
...
**検証**:
```bash
bash $SCRIPT_DIR/assert-issue-state.sh <story-num> --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> \
  --type Story --status "In Planning" --parent <epic-num> --no-iteration
```
...
**検証**:
```bash
bash $SCRIPT_DIR/assert-issue-state.sh <plan-num> --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> \
  --type "Implementation Plan" --status "In Code Review" --parent <story-num> --has-iteration
```

(Plan は起票時に current iteration が auto-assign されるので `--has-iteration`)

加えて親 Story が `Ready` → `In Coding Progress` に自動遷移するかを確認:
```bash
bash $SCRIPT_DIR/assert-issue-state.sh <story-num> --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> \
  --type Story --status "In Coding Progress"
```

> 注: Refinement を経ていないと Story Status が `In Planning` のままで遷移は起きない (これも想定挙動)。事前に手動で Story を `Ready` に上げてから Plan を起票するのが本筋。
...
**検証**:
```bash
bash $SCRIPT_DIR/assert-issue-state.sh <task-num> --repo <owner/repo> \
  --project-owner <ORG> --project-number <NUMBER> \
  --type Task --status Ready --parent <story-num> --has-iteration
```
