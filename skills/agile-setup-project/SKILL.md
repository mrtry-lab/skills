...
- 推奨ビュー 3 つの作成案内 (Backlog / Sprint / Overview)
- Done への遷移ポリシーの決定
...
flowchart TB
  ...
  pre --> it --> proj --> status --> done --> views --> refs --> docs --> smoke --> summary
```
...
| 選択肢 | 内容 |
|---|---|
| 手動で作成する (Recommended) | URL を案内し、ユーザーがブラウザで Backlog / Sprint / Overview の 3 View を作成 |
...
> - 用途: Open な Epic 配下の Story を Epic 別に俯瞰
...
> - 用途: 実装フェーズに入った Story 配下の Implementation Plan / Task を Story 別に追う
>
...
> - 用途: 全 Issue を Type 別に階層付きで俯瞰。Backlog / Sprint より広い視野で「Epic → Story → Plan/Task」のツリーと進捗 (Sub-issues progress) を一覧する

作成完了をユーザーに確認してから次へ。各 View URL (`https://github.com/orgs/<ORG>/projects/<NUMBER>/views/<VIEW_NUMBER>`) を控えると後の完了サマリーに使える。
...
`Read` で `<skill-dir>/references/browser-views.md` を読み込み、`<ORG>` / `<NUMBER>` を実値に置換して Chrome 拡張経由でブラウザ操作を実行。指示書には Parent issue フィールド存在チェックも含まれる。完了後、Backlog / Sprint / Overview の View URL を skill 呼び出し元に返す。失敗したら手動案内に fallback。
...
委譲後、配置完了をユーザーに確認して Step 7.7 へ。

---

## Step 7.7: 動作確認 (Smoke Test)
...
| Status が設定されない | github-projects.json のプレースホルダ未置換 (Step 7 を再実施) |
| Project に追加されない | Auto-add を完全に依存している (Step 5 のスキル明示追加が機能していないので、`agile-create-issue` を再確認) |
| Backlog ビューに Implementation Plan / Task や Epic 単体が表示される | Filter の `type:"Story"` が反映されていない (Step 6 で再設定) |
...
- **動作確認の実施可否** — Step 7.7 で smoke test を回すか、cleanup ポリシーは人間判断
