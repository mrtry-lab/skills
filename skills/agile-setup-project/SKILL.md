...
**期待**:
- Story / Implementation Plan / Task が表示される
- Epic は **表示されない** (type フィルタが効いている)
- Story 配下に Implementation Plan / Task が swimlane で並ぶ
...
動作確認結果:
  ...
  ✓ Backlog ビュー: Epic + Story のみ表示 (type フィルタ動作)
  ✓ Sprint ビュー: Story + Implementation Plan + Task のみ表示
...
| Backlog ビューに Implementation Plan / Task も表示される | Filter の `type:"Epic","Story"` が反映されていない (Step 6 で再設定) |
