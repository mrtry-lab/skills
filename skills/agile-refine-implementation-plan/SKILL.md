...
team-context.md が未配置の場合は **軽量プリセット相当の `USE_CASE` + `INLINE`** をデフォルトとして仮定し、その旨を Implementation Plan に注記する。
...
更新コマンドは `.claude/skills/references/github-projects.md` のテンプレートに従う。失敗時はリトライ 1 回、それでも失敗すれば手動更新を案内して作業を続行 (Status 更新で本作業をブロックしない)。
