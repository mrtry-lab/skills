...
**Status 遷移**: Plan Issue は Step 14 で起票するため、本ステップ時点では Issue 自体がまだ存在しない (Step 14 で `In Code Review` 起票)。
...
Plan は Task と同じ 4 Status (`Ready` / `In Coding Progress` / `In Code Review` / `Done`) を使う。これにより Sprint ビューで Plan も Story 配下に Task と並んで表示され、レビュー進捗を統一的に追える。

### 親 Story の Status 自動更新

Plan Issue 起票完了後、**親 Story の Status を確認して必要なら遷移させる**:

| 現在の親 Story Status | 処理 |
|---|---|
| `Ready` | `In Coding Progress` に更新 (Plan が作成された = 実装フェーズ入り) |
| `In Coding Progress` 以降 | 何もしない (再 Refinement で Plan を作り直すケース) |
| `Ready` 未満 (`In Plan Refinement` 等) | 警告のみ。Refinement 未完了で Plan を作るのは異常ケース |
...
- **絶対に** Plan で PR を出さない — Plan はドキュメントなので PR は出ない。ただし Status 自体は Task と同じ 4 Status を使う (`In Code Review` は「Plan ドキュメントのレビュー」を意味)
