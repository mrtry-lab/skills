...
## 4 リスクチェック + ビジョン整合 (5 並列サブエージェント)

Opportunity Canvas 対話の後、**5 つのサブエージェントを並列起動** して 4 リスクとビジョン整合をそれぞれ独立に評価する。1 つの LLM に 5 観点を同時に持たせると認知が混ざる (Three Amigos と同じ問題) ので、視点ごとに分担する。

主エージェントの責務はオーケストレーション:
1. 5 サブエージェントを **同一メッセージ内で並列起動**
2. 結果を視点別にユーザーに提示 (マージしない)
3. ユーザー回答を踏まえて修正後、未解消視点だけ再起動

### 共通指示部 (各 sub-agent への前置き)

```
以下の Epic Issue の Opportunity Canvas 内容と docs/vision/README.md を読み込み、
あなたに割り当てられた 1 観点だけを検査してください。他の観点には踏み込まないこと。
判定は (低 / 中 / 高リスク) + 根拠を返してください。
```

### Sub-agent A: 価値リスク

```
観点: 価値リスク。ユーザーはこの Epic が提供する価値を選び、実際に使ってくれるか?
- Problem Space のユーザー課題は事実で裏付けられているか、仮説のままか
- 現在の解決策との差分は十分に大きいか
- ユーザーが「これを使いたい」と思う理由が明確か
```

### Sub-agent B: ユーザビリティリスク

```
観点: ユーザビリティリスク。ユーザーは使い方を理解し、使い続けられるか?
- 価値ストーリー / 導入戦略が具体的でステップが追えるか
- 新規ユーザーの最初の成功体験までの摩擦
- 既存導線への組み込み or 別オンボーディングが必要か
```

### Sub-agent C: 実現可能性リスク

```
観点: 実現可能性リスク。今のチーム・技術・時間で作れるか?
- 未経験の技術領域や外部依存の有無
- 既存実装 / インフラとの整合
- チーム規模 / 稼働 (team-context.json の preset) に対するスコープの妥当性
```

### Sub-agent D: 事業継続性リスク

```
観点: 事業継続性リスク。ビジネスとして成立し続けるか?
- ビジネスインパクトに対するコストの見合い
- Outcome 仮説の指標が事業継続性につながるか
- 法務 / セキュリティ / 規約 上の懸念
```
...
```
観点: ビジョン整合。docs/vision/README.md の Layer 1 ミッション / Layer 3 Not-to-do リスト
に照らして整合しているか?
- ミッションに直接貢献するか間接か
- Not-to-do リストの項目に該当していないか
- ペルソナ (Layer 2) と Opportunity Canvas のターゲットユーザーが一致するか
```

### 結果統合 (主エージェント)

5 視点の判定 (低 / 中 / 高 + ビジョン整合の OK / 要確認 / NG) を **マージせず別セクションで提示**。視点間の矛盾 (例: 価値高評価 vs ビジョン NG) もそのまま提示し、採否はユーザーが判断:
...
- 修正後は **該当視点の subagent だけ** 再起動 (OK 判定は再検査しない、context 節約)
