---
name: agile-decompose-task-from-implementation-plan
description: "リファインメント済みの Implementation Plan Issue から Task Sub-issue を起票する。Implementation Plan の Task 分解計画を踏襲。軽量パスで Implementation Plan 不要の Story 場合は Story 入力モードで直接 Task 起票も可能。Use when Implementation Plan から Task 起票、Implementation Plan ベースの Task 分解、軽量 Story の Task 直接起票。Triggers: Implementation Plan から Task 作成, Implementation Plan to Task, Task 起票, タスク分解, Sub-issue作成, 実装タスク分解, PR単位の分割."
---
...
- 軽量パス (Implementation Plan 不要) の Story から直接 Task を起票するとき
- `/agile-decompose-task-from-implementation-plan` で手動実行

## When NOT to Use

- Implementation Plan も Story も未 Refined（→ `/agile-refine-story` または `/agile-refine-implementation-plan`）
- Epic を Story に分解したい（→ `/agile-create-stories`）
- `nature:chaotic` Story → Task 分解不要、`/agile-implement-task` 直行
...
flowchart TB
  input["1. 入力種別判定"]
  plan_mode["2. Implementation Plan ベースモード"]
  story_mode["2'. 軽量モード"]
  ...
  input -- "Implementation Plan Issue 指定" --> plan_mode --> decompose
  input -- "Story Issue (軽量)" --> story_mode --> decompose
  input -- "chaotic" --> chaotic["中断:\n/agile-implement-task 案内"]
  input -- "Implementation Plan 必要だが未作成" --> need_plan["中断:\n/agile-refine-implementation-plan 案内"]
...
| `Awaiting sprint review` | **起票拒否**。受け入れ確認中の Story に新 Task を追加するのは設計上 NG。「先に `/agile-sprint-review` を実行して Story を Done または In Coding Progress に確定してから再度起票してください」と案内して中断 |
| `Done` | **起票拒否**。「`Done` の Story には新規 Task を起票できません」と案内して中断 |
...
| 親 Story Status | 動作 |
|---|---|
| `In Coding Progress` 以降 (ただし `Awaiting sprint review` / `Done` を除く) | Step 2 へ進む (正常) |
...
| `Awaiting sprint review` / `Done` | **起票拒否**。完了確認中 / 確定済みの Story には新規 Task を追加しない |
...
## Step 2: Implementation Plan ベースモードでの Task 起票

GitHub MCP の `issue_read` で **Implementation Plan Issue** を読み込み、本文の「Task 分解 (PR 計画)」セクションを抽出。

Implementation Plan に書かれた Task 一覧をそのまま起票候補とする。Implementation Plan の Task ごとに以下を確認:

- タイトル (Implementation Plan の Task テーブルから取得)
- スコープ (Implementation Plan のスコープ列)
- 依存関係 (Implementation Plan の依存列)
- カバーする受入基準 (Implementation Plan の Strategy / 受入基準マッピングから推定)

Implementation Plan の Task が 10 個超えていたら Implementation Plan 自体に問題がある (`/agile-refine-implementation-plan` での見直しを案内)。

---

## Step 2': 軽量モードでの Task 起票

Story Issue を読み込み、Implementation Plan なしで Task を起票する。
...
軽量モードで Task が 3 個超えそうなら、「これは Implementation Plan 作成パスにすべきです」と提案し中断、`/agile-refine-implementation-plan` を案内する。
...
- **Implementation Plan ベースモード**: Implementation Plan の「Task 分解 (PR 計画)」セクションを起点に、各 Task が親 Story のどの受入基準をカバーするかをマッピング
- **軽量モード**: 親 Story の受入基準を 1つずつ見て、どの Task でカバーするかを直接マッピング

| 分解の起点 | 切り方 |
|-----------|--------|
| Implementation Plan あり | Implementation Plan の Task 分解計画を踏襲 (Implementation Plan で確定済み) |
| Implementation Plan なし軽量モード | Step 2' で読み込んだ team-context の分割パターンに従う (USE_CASE / LAYER / COMPONENT / VERTICAL_SLICE) |
| Implementation Plan なし軽量モード + Infra 変更必要 + `SEPARATE_PR` 設定 | [Migration] / [Infra] を独立 Task として切る。複雑になりすぎなら Implementation Plan 作成パスに引き戻す |
...
| `In Coding Progress` | スキップ (Implementation Plan 経由で既に遷移済み / 2 回目以降の Task 起票) |
...
- 受入基準が曖昧で落とせない → `/agile-refine-story` に差し戻し提案
...
| 状況 | 対応 |
|------|------|
| 親 Story の受入基準が TBD | `/agile-refine-story` に誘導 |
...
- **NEVER: Implementation Plan を作るべき複雑度の Story を軽量モードで強引に処理するな** — Task 数が想定より増えそうなら中断し、`/agile-refine-implementation-plan` を案内する
- **NEVER: Implementation Plan ベースモードで Implementation Plan の Task 分解計画を勝手に書き換えるな** — Implementation Plan が確定済みの分解計画は踏襲する。変更が必要なら Implementation Plan を再 Refinement する
