...
- Step 8 Implementation Plan 必要性判定 (chaotic は常に Implementation Plan 不要)
...
- 観測手段の **具体実装** (GA event 名、カスタムイベント実装の詳細) は Implementation Plan の Step 7 で確定
...
## Step 8: Implementation Plan 必要性判定
...
実際の閾値は `team-context.md` の「Implementation Plan 作成パスの想定 Task 数閾値」「横断的判断閾値」を参照。
...
- **Implementation Plan 不要 (軽量パス)** → 「次は `/agile-implementation-plan-to-task` を実行してください (Story 入力モード)。Story から直接 Task Issue を起票します。」
...
- **Implementation Plan 必要性の最終判断** — Step 8 で AI が判定するが、最終決定は人間
...
- **NEVER: Implementation Plan 必要性判定を独断で決めるな** — Step 8 の判定は人間承認ゲート。AI が「Implementation Plan 不要」と判断しても、人間が「Implementation Plan を作りたい」と言えばそれに従う
