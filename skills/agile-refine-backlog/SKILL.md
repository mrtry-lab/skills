...
- ルール **5 個超え** → Story の責務が広がりすぎ。Step 5f を中断し `/agile-create-backlog` に戻って Story 分割を検討
- 質問 **3 個超え** → リファインメント完了とせず、依頼元（PdO・法務など）への確認を先行。完了は確認待ち
- タイムボックス **25 分超過** → ルールが複雑すぎる兆候。同上
...
9. Outcome Done の整合性: Outcome Done テーブルの観測手段が Step 5c のロギング設計と整合しているか（観測手段を持たない指標が残っていないか）。または `> 観測しない` の判断が明示されているか
10. ビジネスルール網羅: Step 5f で抽出したルールがすべて受入基準に対応しているか。実装時に CodingAgent が見落とす可能性のあるルール（暗黙の権限・上限値・状態遷移など）が残っていないか
