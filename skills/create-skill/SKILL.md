...
これは skill-creator の workflow の Step 4-6 (Test Cases / Run / Improve) に相当。description optimization と package はまだ実行しない (Step 10/11 で扱う)。
...
| Grade < A かつ iter == 5 | Step 13 (ユーザー判断) |
...
## Step 11: (任意) package_skill

`AskUserQuestion`:
- (Recommended) 実行する — `.skill` ファイルを生成、第三者にも配布可能
- skip — 自分で使うだけなら不要

実行を選んだ場合、skill-creator に `python -m scripts.package_skill <path>` を依頼。
...
| 最終ゲート iter | 5 (Step 13 で +5 可) | `最終ゲート iter #N` | Step 6 巻き戻し時もリセットしない (累積) |

Step 13 で「+5」を選ばれたら、TaskList 上に新たな `最終ゲート iter #6` 以降のタスクを生やし、内部カウンタの上限値だけを上書きする (既存タスクは残す、何が起きたか可視化のため)。
...
| Step 10 / 11 | description optimization / package の実行可否 | `AskUserQuestion` |
| Step 13 | ループ上限到達後の方針 | `AskUserQuestion` |
...
- ❌ **Grade A 達成後の勝手なループ継続** — Step 10/11/12 へ進んで完了させる
