...
最後に `### LLM 環境では確認できなかった項目`（または同等の見出し）を 1 つ置き、人間レビュアーに任せる項目をまとめる。
...
3. LLM が確認できないものは `- [ ]` のまま残し、`### LLM 環境では確認できなかった項目` セクションに分離する
...
5. 確認できなかったもの（実機 / 視覚 / 仕様判断等）は `### LLM 環境では確認できなかった項目` セクションに移し、`- [ ]` のまま残す（依頼文に書き換えない）
...
- [ ] `### LLM 環境では確認できなかった項目`（または同等の見出し）が存在し、人間レビュアー対象がそこに分離されている
- [ ] 各 AC が「〜できる」「〜することができる」のユーザーストーリー / 受け入れ基準形式で書かれている
- [ ] 「〜してほしい」「〜してください」などの依頼文を使っていない（人間レビュアー対象でも AC 形式を保つ）
- [ ] チェックリスト形式（`- [x]` / `- [ ]`）になっている
