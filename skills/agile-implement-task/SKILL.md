...
| 5 | **エスカレーション識別** | 計画外 API / インフラ変更 / ADR 矛盾など Step 5 で停止すべきリスクを事前列挙 |
| 6 | **完了条件の判定可能性** | Task Issue の完了条件を Yes/No で判定可能と確認 |

**6 点中 5 点以上で合格。4 点以下は書き直し。** AI が自己採点した結果（各観点の判定と理由）を Step 4 でナビゲーターに提示する。承認/差し戻しはナビゲーターが判断する。

---

## Step 4: ナビゲーター承認

Plan mode を抜けてナビゲーターに計画のレビューを求める。修正指示 → Step 2 へ。承認 → Step 5 へ。
...
- **Plan mode の計画承認** — Step 4 のナビゲーター承認なしに Step 5 実装に進まない（既存の運用と一致）
- **エスカレーション該当時の停止** — Step 5 のエスカレーション表に該当する状況（API 設計が必要 / ADR と矛盾 / 計画 2 倍超 / 仕様曖昧）は AI が独断せずナビゲーターに報告
