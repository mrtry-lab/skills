---
name: agile-craft-vision
description: "プロダクトビジョンドキュメント（docs/vision/README.md）を対話的に作成・更新する。チームの前提認知を揃えるためのWhy/Who/What/How/When-Risk 5層構造で、ミッション・エレベーターピッチ・ペルソナ・Not-to-doリスト・トレードオフスライダー・リスクリストを網羅する。Use when creating or updating product vision, aligning team premises, defining product direction, or starting a new product. Triggers: ビジョン作成, ビジョン更新, docs/vision/README.md, product vision, エレベーターピッチ, ペルソナ定義, トレードオフ, Not-to-doリスト, プロダクトの方向性, inception deck, チーム前提を揃えたい."
---
...
- チーム内で前提認知のズレを感じたとき
- `/agile-craft-vision` で手動実行

## When NOT to Use

- Epic/Featureの具体的な定義（→ `/agile-create-epic`）
- Story 分解（→ `/agile-create-stories`）
...
```mermaid
flowchart TB
  mode{"docs/vision/README.md\n存在する?"}
  create["1. 新規: 5層を順に対話"]
  update["1u. 更新: 変更層を特定"]
  dialogue["2. 層ごとの問いかけ + 回答統合"]
  write["3. docs/vision/README.md 生成/更新"]
  review["4. ビジョン整合レビュー\n(サブエージェント)"]
  done["5. チームレビュー依頼"]
...
- **新規作成**: `docs/vision/README.md` が存在しない、または空 → 5層すべてを順に対話
- **更新**: 既存docs/vision/README.mdを読み込み → ユーザーに「どの層を見直したいか」を確認 → 該当層のみ対話
...
### `docs/vision/` ディレクトリの使い方

`docs/vision/` は **人間が参考資料を整理しておく置き場**。Vision 本体は `docs/vision/README.md` のみで、他の skill (`/agile-create-epic` 等) もこのファイルだけを参照する。サブディレクトリや PDF などは Vision 策定時の根拠を **あとから人間が見返したいとき** のための保管場所:

```
docs/vision/
├── README.md          # 本体。他 skill が読むのはここだけ
├── references/        # 参考資料 (任意、自由構成)
│   ├── user-interview-2026q1.md
│   └── competitor-analysis.pdf
└── research/          # 調査メモ (任意)
    └── market-trends.pdf
```

このスキルはユーザーから対話で得た情報を元に `README.md` を作る。参考資料は **AI が自動 ingestion するのではなく**、ユーザーが必要に応じて対話の中で明示的に引用する想定 (「あの資料に書いた競合の数値を反映したい」等)。

---

## 5層構造 — 対話ガイド

> 閾値（タイムボックス・ペルソナ数等）は `.claude/skills/references/team-context.json` を参照する。設定がなければ「軽量プリセット」（副業チーム想定）をデフォルトに動く。
...
## Step 3: docs/vision/README.md 生成/更新

- **新規作成時**: **MANDATORY** `references/vision-template.md` を読み込み、テンプレート構造に従って出力する。未決セクションは `> TBD: ...` で残す
- **更新時**: テンプレートの再読み込みは不要。既存docs/vision/README.mdの該当セクションのみ書き換える
- **Do NOT Load**: 対話フェーズ（Step 1-2）ではテンプレートを読むな。ユーザーの回答がテンプレートの枠に引きずられることを防ぐ

## Step 4: ビジョン整合レビュー + Strategy 点検（サブエージェント x 2 並列）

docs/vision/README.md 生成後、**2 つのサブエージェントを並列起動**し、それぞれ独立した視点で検査する。1 つの LLM に「内的整合性」と「戦略性質」を同時に持たせると認知が混ざるため、視点ごとに分担する（Three Amigos と同じ方針）。
...
生成された docs/vision/README.md を読み込み、以下を検査してください:
...
生成された docs/vision/README.md を読み込み、以下 4 性質を検査してください:
...
各性質の判定（OK / 要確認 / NG）と、要確認・NG の場合は docs/vision/README.md の具体的
箇所と理由を返してください。
```
...
| 2 | **Layer 2 ペルソナ妥当性** | `team-context.json` のペルソナ上限内、生活感ある記述 |
...
**8 点中 7 点以上で合格。6 点以下は書き直し。** ユーザーに各観点のスコアを提示し、書き直しが必要なら対話を再開する。合格なら Step 6 へ。
...
| 1回の対話で全層を完了できない | 途中でdocs/vision/README.mdを生成し、残りをTBDに。次回セッションで継続 |

## NEVER — アンチパターン

- **NEVER: docs/vision/README.mdを勝手に書くな** — エージェントが独自にビジョンを創作すると、チームが「自分たちのもの」と感じなくなり、ドキュメントが死文化する
- **NEVER: Not-to-doリストを省略するな** — 稼働時間が限られるチームで最も起きやすい失敗は「全部やろうとして何も完成しない」。やらないことの合意がスコープ管理の生命線
- **NEVER: ペルソナを team-context.json の上限を超えて作るな** — `team-context.json` で採用したプリセット値（軽量: 1-2 / 標準: 2-3 / 集中: 3-5）を超えると「誰のために作っているか」が曖昧になる。設定がなければ軽量プリセット（1-2）をデフォルトに
