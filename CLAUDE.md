...
### Subagent 並列実行で認知負荷を下げる

`Agent` ツール (Claude Code の subagent) を使うと、独立した視点の検査を **並列で走らせて context を分離** できる。主エージェントは結果統合と意思決定に集中し、各視点の細かい論理は subagent に任せる。

**並列 subagent 化が効くパターン**:

1. **視点が独立する検査・レビュー** — Three Amigos (PdO / Dev / QA)、4 リスクチェック (価値 / UX / 実現可能性 / 事業) など、観点が直交していてマージすると質が落ちる場合。1 つの LLM に複数視点を持たせると認知が混ざるので、視点ごとに subagent を分ける
2. **画面 / 設定 / 状態の並列確認** — 例: Backlog / Sprint / Overview の 3 ビュー検査。3 ビューは独立して評価可能で、結果が出揃ったらまとめてユーザーに提示
3. **context を汚す探索** — リポジトリ内の類似実装を Read / Grep で大量に読む場合、`Explore` subagent に投げて要点だけ受け取ると主エージェントの context が軽くなる

**主エージェントの責務**:
- 同一メッセージ内で複数の `Agent` ツール呼び出しをまとめて、**真の並列実行** にする (逐次起動は遅い + 同期コスト)
- 各 subagent には **1 つの観点だけ** を持たせる。「PdO 視点と QA 視点を兼ねて検査して」のようなマージ指示は避ける
- 結果をマージしない。視点別に並べて提示し、矛盾はそのまま論点として出す
- 修正後の再検査は **未解消視点だけ** 起動。OK 判定の視点を再起動しない (context 節約)

**避けるべきケース**:
- ステップが順序依存 (前 step の結果が次 step の入力)
- 観点が 2 つしかない / 計算量が小さい (subagent 起動オーバーヘッドが効果を上回る)
- 自由対話が必要なフェーズ (subagent はユーザー対話に向かない)

実装例: `skills/agile-craft-vision` Step 4 (2 並列) / `agile-refine-story` Step 8 (3 並列) / `agile-refine-implementation-plan` Step 14 (3 並列) / `agile-create-epic` 4 リスクチェック (5 並列) / `agile-setup-project` smoke test view 確認 (3 並列)。
