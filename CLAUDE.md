...
### Chrome 拡張 (Claude for Chrome 等) でのブラウザ自動化

GitHub Projects v2 の Workflow 設定 / View 作成、Organization の Issue Type 登録など、**Web UI でしか操作できない箇所**がある。これらは Claude for Chrome 等のブラウザ拡張経由で自動化できる可能性がある。

skill 側の正しい組み立て:

1. まず手動操作を案内する (URL + 手順)
2. `AskUserQuestion` で「Chrome 拡張で自動化しますか?」を聞く
3. 自動化を選ばれたら、skill 同梱の `references/browser-*.md` を `Read` して指示書を取り出し、Chrome 拡張に渡して実行させる
4. 失敗したら手動案内に fallback
